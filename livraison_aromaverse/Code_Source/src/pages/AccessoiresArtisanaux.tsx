      ...
      <main className="flex-1 pt-28 pb-16 container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-12 flex flex-col md:flex-row items-center justify-between gap-8 border-b border-white/5 pb-8">
          <div className="w-full md:w-80 relative">
            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-3 pl-1">Catégorie Traditionnelle</label>
          ...
          <div className="flex items-center gap-4 px-6 h-14 rounded-2xl bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-gold italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredItems.map((item, i) => (
             <div key={i} className="glass-card rounded-[40px] overflow-hidden border border-white/5 bg-black/40 flex flex-col hover:border-gold/30 transition-all hover:scale-[1.02] duration-500 group relative shadow-2xl">
                <div className="aspect-square relative overflow-hidden m-4 rounded-[32px]">
                   <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-all z-10" />
                   <img src={item.img} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 bg-black" alt={item.title} />
                   <div className="absolute top-6 right-6 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full border border-white/10 z-20 text-[8px] font-black text-white uppercase tracking-widest">
                      {item.category}
                   </div>
                </div>
                <div className="p-8 flex flex-col flex-1">
                   ...
                   <div className="flex justify-between items-center mt-auto pt-6 border-t border-white/10">
                     <div>
                        <span className="text-[8px] font-black uppercase text-muted-foreground block">Prix Unitaire</span>
