        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          {showProposal && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setShowProposal(false)}></div>
              <div className="relative bg-[#062112] border border-gold/30 p-10 rounded-[40px] w-full max-w-lg shadow-2xl animate-in zoom-in duration-300">
                <h2 className="text-3xl font-display font-black text-white mb-2 uppercase tracking-tight">Proposer une Culture</h2>
                ...
                <form onSubmit={handlePropose} className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Que souhaitez-vous planter ?</label>
                    <input 
                      type="text" 
                      placeholder="Ex: Rose de Damas, Oud, Jasmin..." 
                      className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl px-6 text-white text-sm focus:border-gold outline-none transition-all"
                      value={proposalData.crop}
                      onChange={e => setProposalData({...proposalData, crop: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Surface estimée (Hectares)</label>
                    <input 
                      type="number" 
                      placeholder="Ex: 5" 
                      className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl px-6 text-white text-sm focus:border-gold outline-none transition-all"
