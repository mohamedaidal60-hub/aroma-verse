        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              ...
              <p className="text-xl text-emerald-700/70 mt-2">{t("dashboard.activity")}</p>
            </div>
            ...
            <div className="flex items-center gap-4 bg-emerald-50 p-4 rounded-3xl border border-emerald-100">
                ...
                <div>
                  <p className="text-xs text-emerald-700/70 uppercase font-bold tracking-widest">{t("dashboard.plan")}</p>
                  <p className="text-foreground font-black">{(profile as any)?.current_plan === 'pro' ? t("dashboard.plan.pro") : t("dashboard.plan.trial")}</p>
                </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
             {[
               ...
               { icon: BrainCircuit, label: t("dashboard.courses"), value: stats.course_count, sub: "Cours en cours", color: "bg-purple-500/10 text-purple-500" },
                ...
                <div key={i} className="glass-card p-6 rounded-[32px] border border-emerald-100 relative group hover:border-gold/30 transition-all">
                  ...
                  <div className="text-xs text-emerald-700/70 font-bold uppercase tracking-tight">{stat.label}</div>
                  <span className="text-[10px] text-emerald-700/70/60">{stat.sub}</span>
          ...
          <div className="glass-card mb-16 p-6 rounded-[32px] border border-green-500/30 bg-green-500/5 flex flex-col md:flex-row items-center justify-between gap-6 hover:bg-green-500/10 transition-colors">
            <div className="flex items-center gap-4 text-green-500">
               ...
               <div>
                 ...
                 <p className="text-sm text-emerald-700/70">Transformez vos points de formulation en <strong className="text-green-500 font-black">Actions ou Parts d'Investissement Agricole</strong>.</p>
               </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                 <span className="text-2xl font-black text-foreground block leading-none">2,450 PTS</span>
                 <span className="text-[10px] uppercase tracking-widest text-emerald-700/70">= 2 Actions Lavande</span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 flex flex-col gap-10">
               ...
               <section>
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <Button variant="ghost" className="text-emerald-700/70 font-bold" onClick={() => navigate('/marketplace')}>{t("common.see_all")}</Button>
                  </div>

                  {orders.length === 0 ? (
                    <div className="glass-card p-10 rounded-[32px] border-dashed border-emerald-200 text-center">
                       <ShoppingBag size={40} className="mx-auto text-emerald-700/70/20 mb-4" />
                       <p className="text-emerald-700/70 mb-6">{t("common.no_data")}</p>
                    ...
                    <div className="space-y-4">
                      {orders.map(order => (
                        <div key={order.id} className="glass-card px-8 py-6 rounded-[32px] border border-emerald-100 hover:border-emerald-200 transition-all flex items-center justify-between">
                           <div className="flex items-center gap-6">
                              <div className="w-14 h-14 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-700/70">
                                <Package size={24} />
                              </div>
                              <div>
                                 <p className="font-bold text-foreground leading-none mb-1 text-lg">Commande #{order.id.toString().slice(0, 8)}</p>
                                 <div className="flex items-center gap-3 text-xs text-emerald-700/70">
                           ...
                           <div className="text-right">
                              <div className="text-xl font-black text-foreground">{order.total_amount || 0} €</div>
               ...
               <section>
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <Button variant="ghost" className="text-emerald-700/70 font-bold" onClick={() => navigate('/studio')}>{t("community.new_post")}</Button>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     {formulas.length === 0 ? (
                       <div className="glass-card p-10 rounded-[32px] border-dashed border-emerald-200 text-center md:col-span-2">
                          <Palette size={40} className="mx-auto text-emerald-700/70/20 mb-4" />
                          <p className="text-emerald-700/70 mb-6">{t("common.no_data")}</p>
                          <Button className="bg-gold text-black font-bold h-12 rounded-2xl px-8" onClick={() => navigate('/studio')}>{t("nav.studio")}</Button>
                       </div>
                     ) : (
                       formulas.map(formula => (
                         <div key={formula.id} className="glass-card p-6 rounded-[32px] border border-emerald-100 hover:border-gold/20 transition-all group">
                             ...
                             <p className="text-xs text-emerald-700/70 leading-relaxed mb-6">{formula.description || 'Formule olfactive enregistrée'}</p>
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
               ...
               <div className="glass-card p-10 rounded-[48px] border-2 border-gold/30 bg-gradient-to-br from-gold/15 to-transparent relative overflow-hidden">
                  ...
                  <p className="text-sm text-emerald-700/70 leading-relaxed mb-8">
               ...
               {/* Community Vote CTA */}
               <div className="glass-card p-6 rounded-[40px] border border-emerald-100">
                 <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-purple-500/10 flex items-center justify-center text-purple-500">
                      <Globe size={24} />
                    </div>
                    <div>
                       <h4 className="font-black text-xs uppercase tracking-widest text-emerald-700/70">{t("footer.ecosystem")}</h4>
                       <p className="font-bold text-foreground">Classement des Nez</p>
                    </div>
                 </div>
                 <p className="text-xs text-emerald-700/70 leading-relaxed mb-6">Votez pour les 10 meilleurs créateurs de ce trimestre et gagnez des points Nexus.</p>
                 <Button variant="outline" className="w-full rounded-2xl border-emerald-200 hover:border-gold/50 h-12 font-bold" onClick={() => navigate('/community')}>
                    Aller aux Votes
                 </Button>
               </div>
               ...
               <div className="glass-card p-8 rounded-[40px] border border-gold/30 bg-gold/5 relative overflow-hidden mt-2 group">
                  ...
                  <p className="text-sm text-emerald-700/70 leading-relaxed mb-6 relative z-10">
