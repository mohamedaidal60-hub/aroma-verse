              ...
              {items.map((item) => (
                <div key={item.id} className="glass-card rounded-2xl p-4 group cursor-pointer hover:border-primary/50 transition-colors flex flex-col">
                  <div className="aspect-square bg-secondary rounded-xl mb-4 overflow-hidden relative flex text-primary/10 items-center justify-center">
                    <img 
                      src="https://images.unsplash.com/photo-1588405748880-12d1d2a59f75?auto=format&fit=crop&w=400&q=80" 
                      alt={item.title} 
                      className="w-full h-full object-cover transition-all"
                      style={{
                        filter: item.title.toLowerCase().includes("oud") ? "hue-rotate(330deg) saturate(1.5) brightness(0.8)" : 
                               item.title.toLowerCase().includes("flacon") || item.title.toLowerCase().includes("design") ? "grayscale(0.9) contrast(1.2)" :
                               item.title.toLowerCase().includes("musc") ? "none" : 
                               "hue-rotate(200deg) saturate(0.8) brightness(1.1)"
                      }}
                    />
