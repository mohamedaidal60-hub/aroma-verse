    ...
    <div className="min-h-screen bg-background font-body">
      ...
      <div className="pt-24 pb-16">
        <div className="container mx-auto px-4 max-w-7xl">
            ...
            <div className="flex gap-4 w-full md:w-auto self-end">
               {isSpectator ? (
                  <div className="bg-white/5 border border-white/5 p-4 rounded-2xl flex items-center gap-3">
                     ...
                     <div className="text-[10px] uppercase font-black text-muted-foreground tracking-widest leading-tight">
          ...
          {/* Filtres Catégories */}
          <div className="flex gap-3 overflow-x-auto pb-6 mb-10 scrollbar-hide border-b border-white/5">
            {categoryFilters.map((filter) => (
              <Button 
                key={filter} 
                variant={activeFilter === filter ? "default" : "secondary"} 
                className={`rounded-xl px-6 h-12 whitespace-nowrap transition-all border ${activeFilter === filter ? "bg-gold text-black border-gold font-black" : "bg-white/5 border-white/5 hover:bg-white/10 text-muted-foreground"}`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
             {items.filter(item => {
                const matchesCategory = activeFilter === "Tous" || item.category === activeFilter;
                ...
                <div key={item.id} className="glass-card rounded-[32px] p-6 group cursor-pointer hover:border-gold/30 transition-all flex flex-col relative" onClick={() => handleOpenChat(item)}>
                   <div className="aspect-[4/5] bg-secondary/30 rounded-2xl mb-4 overflow-hidden relative border border-white/5">
                      <img 
                        src={item.image} 
                      ...
                      />
                      <div className="absolute top-3 left-3 px-3 py-1 bg-black/60 backdrop-blur text-white text-[10px] font-black uppercase tracking-widest rounded-full">
                         {item.category}
                      </div>
                   </div>
                   <h3 className="font-display font-medium text-xl mb-1 line-clamp-1 text-white">{item.title}</h3>
                   ...
                   <div className="mt-auto flex justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/5">
                      <div>
                        <span className="text-[10px] text-muted-foreground uppercase block">Prix Public</span>
                   ...
                   {isSpectator && (
                      <div className="absolute inset-x-6 bottom-20 py-2 bg-black/60 backdrop-blur-md rounded-xl flex items-center justify-center gap-2 border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {/* Modal Vente */}
          {showAddModal && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg" onClick={() => setShowAddModal(false)}>
                <div className="glass-card max-w-2xl w-full p-8 rounded-[40px] border border-white/10 shadow-2xl overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <h2 className="text-4xl font-display font-black text-white mb-2">Vendre un Article</h2>
                    <p className="text-muted-foreground text-sm mb-8">Partagez votre stock avec la communauté. Nous appliquons 20% de commission.</p>
                    
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                           <div>
                              <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-2 block">Nom de l'article</label>
                              <Input className="h-12 bg-white/5 rounded-xl border-white/10 text-white placeholder:text-white/20" placeholder="Ex: Absolu Jasmin de Grasse" value={newProdName} onChange={e => setNewProdName(e.target.value)} />
                           </div>
                           <div>
                              <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-2 block">Catégorie</label>
                              <select className="w-full h-12 bg-white/5 border border-white/10 rounded-xl text-white px-4 text-sm" value={newProdCategory} onChange={e => setNewProdCategory(e.target.value)}>
                                 {categoryFilters.filter(c => c !== 'Tous').map(c => <option key={c} value={c} className="bg-black">{c}</option>)}
                              </select>
                           </div>
                        </div>

                        <div>
                           <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-2 block">Descriptif Technique & Olfactif</label>
                           <textarea className="w-full bg-white/5 rounded-xl border-white/10 text-white p-4 min-h-[100px] text-sm focus:border-gold outline-none transition-all" placeholder="Détaillez l'origine, la pureté et les notes..." value={newProdDesc} onChange={e => setNewProdDesc(e.target.value)}></textarea>
                        </div>
                        
                        <div className="flex gap-4 items-end">
                           <div className="flex-1">
                              <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-2 block">VOTRE PRIX NET (€)</label>
                              <Input type="number" className="h-14 bg-white/10 rounded-xl border-gold/30 text-white text-2xl font-black text-center" placeholder="0.00" value={newProdPrice} onChange={e => setNewProdPrice(e.target.value)} />
                           </div>
                        ...
                        <div>
                           <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-2 block">Photos (Max 5 images)</label>
                           <div className="flex gap-3 flex-wrap">
                              {previewImages.map((src, i) => (
                                 <div key={i} className="w-20 h-20 rounded-xl overflow-hidden relative border border-white/20 shadow-lg">
                                    <img src={src} className="w-full h-full object-cover" />
                                    <button onClick={() => setPreviewImages(previewImages.filter((_, idx)=>idx!==i))} className="absolute top-1 right-1 bg-black/80 rounded-full p-1 text-white hover:text-red-500 transition-colors"><X size={12}/></button>
                                 </div>
                              ))}
                              {previewImages.length < 5 && (
                                 <label className="w-20 h-20 rounded-xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-muted-foreground cursor-pointer hover:border-gold hover:text-gold transition-all bg-white/5">
                        ...
                        <div className="pt-4 flex gap-4">
                           ...
                           <Button variant="outline" className="flex-1 h-14 border-white/10 text-white rounded-2xl" onClick={() => setShowAddModal(false)}>Annuler</Button>
                        </div>
                    </div>
                </div>
            </div>
          ...
          {selectedProduct && !isChatMinimized && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl" onClick={() => {setSelectedProduct(null); setShowChat(false)}}>
                <div className="glass-card max-w-6xl w-full flex flex-col md:flex-row overflow-hidden rounded-[40px] border border-white/10 shadow-3xl bg-[#0a0a0a]" style={{ maxHeight: '90vh' }} onClick={e => e.stopPropagation()}>
                    
                    {/* Colonne Produit */}
                    <div className="w-full md:w-1/2 p-8 md:p-12 border-b md:border-b-0 md:border-r border-white/5 overflow-y-auto scrollbar-hide">
                        <div className="aspect-square w-full rounded-[32px] overflow-hidden mb-10 border border-white/5 bg-secondary/20 shadow-2xl relative group/img">
                           <img 
                           ...
                           />
                           <div className="absolute top-6 right-6 px-4 py-2 bg-black/80 backdrop-blur rounded-2xl border border-white/10 flex items-center gap-2">
                              <ShieldAlert size={14} className="text-gold" />
                              <span className="text-white font-black text-[10px] uppercase tracking-widest">Contrôlé AromaVerse</span>
                           </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mb-6">
                           ...
                           <div>
                              ...
                              <p className="text-white font-bold">{selectedProduct.vendor}</p>
                           </div>
                        </div>

                        <h2 className="text-4xl font-display font-black text-white mb-4 tracking-tighter">{selectedProduct.title}</h2>
                        <div className="flex items-baseline gap-2 mb-10">
                           <div className="text-5xl font-black text-gold">{selectedProduct.displayPrice.toFixed(2)} €</div>
                           <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Prix Final Public</span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-10">
                           <div className="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                              ...
                              <span className="text-white text-sm font-bold uppercase tracking-tight">{selectedProduct.category}</span>
                           </div>
                           <div className="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                              <span className="text-[9px] uppercase font-black text-gold/60 block mb-1">ID Objet</span>
                              <span className="text-white text-sm font-mono">#{selectedProduct.id.toString().slice(-6)}</span>
                           </div>
                           <div className="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                           ...
                           <div className="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                              ...
                              <span className="text-white text-sm font-bold">En Stock</span>
                           </div>
                        </div>
                        ...
                        <div className="space-y-4 mb-12">
                           <div className="flex items-center gap-2 border-b border-white/10 pb-2 mb-4">
                              ...
                              <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Note du Vendeur</h4>
                           </div>
                           <p className="text-white/60 leading-relaxed text-sm italic">"{selectedProduct.desc}"</p>
                        </div>
                        ...
                        {!isSpectator ? (
                           ...
                           <div className="p-8 bg-red-500/5 rounded-3xl border border-red-500/20 text-center backdrop-blur-lg">
                              ...
                              <p className="text-[10px] text-muted-foreground uppercase leading-relaxed font-medium">L'activation de votre pack membre est nécessaire pour négocier avec ce vendeur.</p>
                           </div>
                        )}
                    </div>
                    ...
                    {showChat && (
                      <div className="w-full md:w-1/2 flex flex-col bg-black/60 relative">
                         ...
                         <div className="p-8 border-b border-white/5 bg-black/80 backdrop-blur-2xl flex items-center justify-between z-10">
                            <div className="flex items-start gap-5">
                               ...
                               <div>
                                  ...
                                  <p className="text-[10px] text-muted-foreground leading-relaxed max-w-sm font-bold uppercase tracking-tight">
                                     ÉCHANGE DE COORDONNÉES INTERDIT.
                                  </p>
                               </div>
                            </div>
                            <Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={() => setIsChatMinimized(true)}>
                               <X size={20} />
                            </Button>
                         </div>
                         ...
                         <div className="flex-1 p-8 overflow-y-auto space-y-8 z-10 scrollbar-hide">
                            <div className="flex justify-center">
                               <div className="px-5 py-2 bg-white/5 rounded-full border border-white/5 text-[9px] font-black uppercase tracking-widest text-gold/40 backdrop-blur-md">
                                  Discussion P2P - Session Chiffrée
                               </div>
                            </div>
                            
                            {chatMessages.length === 0 ? (
                               <div className="h-full flex items-center justify-center text-muted-foreground text-sm flex-col opacity-20 py-20 grayscale">
                               ...
                               chatMessages.map((msg, i) => (
                                 <div key={i} className={`flex flex-col ${msg.sender === 'Moi' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-4 duration-500`}>
                                    <div className={`max-w-[85%] p-6 rounded-[28px] text-sm leading-relaxed shadow-2xl backdrop-blur-md ${msg.sender === 'Moi' ? 'bg-gold text-black font-bold rounded-tr-none' : 'bg-white/10 text-white rounded-tl-none border border-white/10'}`}>
                                       {msg.text}
                                    </div>
                         ...
                         <div className="p-8 border-t border-white/5 bg-black/80 backdrop-blur-3xl z-10">
                             <div className="relative flex items-center">
                                <Input 
                                   className="h-16 flex-1 bg-white/5 border-white/10 rounded-2xl px-8 focus:border-gold transition-all text-sm pr-16 shadow-2xl focus-visible:ring-0 placeholder:text-muted-foreground/30" 
          ...
          {selectedProduct && isChatMinimized && (
            ...
            >
               <div className="absolute -top-1 -right-1 w-6 h-6 bg-red-500 rounded-full border-4 border-background flex items-center justify-center">
                  <span className="text-[10px] font-black text-white">{chatMessages.length || "1"}</span>
               </div>
               <MessageSquare size={32} className="text-black" />
               <div className="absolute -left-48 bg-black/80 backdrop-blur px-4 py-2 rounded-xl text-[10px] font-black text-gold uppercase tracking-widest border border-gold/20 opacity-0 hover:opacity-100 transition-opacity">
