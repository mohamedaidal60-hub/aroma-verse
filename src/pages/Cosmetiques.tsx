...
const Cosmetiques = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col font-body text-foreground">
      <Navbar />
      <main className="flex-1 pt-28 pb-16 container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-16 text-center flex flex-col items-center">
            ...
            <p className="text-emerald-700/70 max-w-2xl mx-auto mb-10 text-lg leading-relaxed">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <div className="flex items-center gap-2 px-6 h-14 rounded-2xl bg-emerald-50 border border-emerald-200 text-xs font-bold uppercase tracking-widest text-emerald-700/70 whitespace-nowrap">
        ...
        <div className="mb-12 flex flex-col md:flex-row items-center justify-between gap-8 border-b border-emerald-100 pb-8">
          <div className="w-full md:w-80 relative group">
            <label className="text-[10px] font-black uppercase tracking-widest text-emerald-700/70 block mb-3 pl-1">Filtrer par catégorie</label>
          ...
          <div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-700/70 flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredCosmetics.map((item, i) => (
            <div key={i} className="glass-card rounded-[40px] overflow-hidden border border-emerald-100 bg-white/40 flex flex-col hover:border-gold/40 transition-all group duration-500 shadow-elevated">
               <div className="absolute top-6 left-6 z-20">
                  <div className="bg-white/60 backdrop-blur-xl px-4 py-1.5 rounded-2xl border border-emerald-100 shadow-xl">
               ...
               <div className="aspect-[4/5] relative overflow-hidden m-3 rounded-[32px]">
                  ...
                  <img 
                    src={item.img} 
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 bg-white" 
                    alt={item.title} 
                    loading="lazy"
                  />
                  <div className="absolute bottom-6 left-6 z-20 flex items-center gap-2">
                    <span className="px-3 py-1 bg-emerald-100 backdrop-blur-md rounded-full text-[8px] font-black text-foreground uppercase tracking-widest border border-emerald-200">
                      {item.category}
                    </span>
                  </div>
               </div>
               ...
               <div className="p-8 flex flex-col flex-1">
                  ...
                  <p className="text-sm text-emerald-700/70 mb-8 flex-1 leading-relaxed opacity-80">{item.desc}</p>
                  
                  <div className="mt-auto flex justify-between items-center bg-emerald-50 p-5 rounded-[24px] border border-emerald-100 group-hover:bg-emerald-100 transition-all">
                    <div>
                      <span className="text-[8px] font-black uppercase text-emerald-700/70 block mb-0.5">Prix d'échange</span>
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2 text-emerald-700/70 opacity-40 group-hover:opacity-80 transition-opacity">
                    <User size={12} />
        ...
        {filteredCosmetics.length === 0 && (
          <div className="py-24 text-center">
            <ShoppingBag size={64} className="mx-auto text-foreground/5 mb-6" />
            <h3 className="text-2xl font-display font-bold text-foreground mb-2">Aucun produit trouvé</h3>
            <p className="text-emerald-700/70">Désolé, aucune annonce ne correspond à cette catégorie pour le moment.</p>
          </div>
        )}
      </main>
      <Footer />
